...
def get_top_words_slow(frequency, top_n=10):
    items = list(frequency.items())
    items.sort(key=lambda x: x[1], reverse=True)
    return items[:top_n]


def analyze_text_slow(text):
    start = time.perf_counter()
    frequency = count_words_slow(text)
    top_words = get_top_words_slow(frequency)
...
def profile_run():
    ...
    print("\n--- PROFILE OUTPUT ---\n")
    print(output)
    with open("profile_baseline.txt", "w", encoding="utf-8") as f:
        f.write(output)
